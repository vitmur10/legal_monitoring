...
Якщо кнопку натиснути до Telegram-публікації, бот пояснює потрібний порядок у чаті.
Після спроби додавання бот надсилає в чат результат, а не лише коротке callback-повідомлення.
...
та «Дата публікації джерела». Дата документа береться з analysis, а дата публікації джерела
зберігається окремо; невідому дату документа не підміняємо датою публікації.
NOTION_PROPERTY_MAP може перевизначити назви полів. Токен і database ID беруться з env.
Для оновлення відсутніх полів на вже створеній базі виконайте
`python scripts/notion_ensure_schema.py`; скрипт перевіряє/додає поля й не створює сторінок.
